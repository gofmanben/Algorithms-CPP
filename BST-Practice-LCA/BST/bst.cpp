...
// Determine the LCA (least common ancestor) of two int values (not two nodes). If either of the values is not present in the BST, output NULL.
// Otherwise, this outputs the LCA of the two values. 
// Input: int val1, int val2, for the two values 
// Output: pointer to the LCA. Output NULL if either val1 or val2 is not present in the tree
Node* BST :: lca(int val1, int val2)
{
    ...
    Example:
        ...
        lca(50,70) = 60
    */
    // If either value is not present, return NULL
    if (!find(val1) || !find(val2)) // (val1 = 10, val2 = 30)
        return NULL;

    Node* curr = root; // 40 20 10 30 60 50 70
    while (curr != NULL)
    {
		int val = curr->key;
        if (val1 < val && val2 < val) {        // 0-[10 < 40 && 30 < 40], 1-[10 < 20 && 30 < 20]
            curr = curr->left;                 // 0-[curr = 20]
        } else if (val1 > val && val2 > val) { // 1-[10 > 20 && 30 > 20]
            curr = curr->right;
        } else {
            break;                             // 1-[curr = 20]
        }
