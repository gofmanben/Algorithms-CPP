...
int main(int argc, char** argv)
{
    ...
    while(getline(input,command)) // get next line of input, store as repeat
    {
        ...
        if(strcmp(op,"lca") == 0) // get the lca
        {
            // get the second value for calling lca
            valstr2 = strtok(NULL, " \t"); // get the next token as a string
            if(valstr2 != NULL) // something is actually there
                val2 = strtol(valstr2, &dummy, 10); // convert into int
            else // didn't find the second argument for lca
                continue; // ignore this line
            Node* lca_node = myBST.lca(val,val2); // call the LCA function
            string out_str; // string for output
            if(lca_node != NULL)
                out_str = to_string(lca_node->key);
            else // lca was null
                out_str = "null";
            output << out_str << endl; // write to file
            cout << "lca of "+to_string(val)+" "+to_string(val2) << endl;
            cout << out_str << endl; // write to console
        }
