...
{
    /*
    Example: heap: [x, 42, 40, 30, 20, 10] (A[0] unused)
            A[1]=42 (current=1)
            /     \
         A[2]=40   A[3]=30
          /   \
    ...
    */
    ...
    // int& lastValue = A[size];  // 42 40 30 20 10 <-
    // A[1] = lastValue;          // 42 -> 10 (move last to root)
    // size--;                    // size reduces by 1 (5 -> 4)
    A[cur] = A[size--];           // Replace with last element and decrease size
    

    // Heapify down using for loop
    while (true)
    {
        // Left child index
        int index = cur * 2;  // 2 = 40, 4 = 20, 8 = NULL

        // If right child exists and is larger, use it instead
        if (index < size && A[index] < A[index + 1]) { // 40 < 30, (index >= size)
            // Right child index
            index++;
        }

        // If parent is already larger than the biggest child, heap property holds
        if (index > size || A[index] < A[cur])  { // 10 >= 40, 10 >= 20
            break; // (index > size)
        }

        // Otherwise, swap parent with larger child
        swap(A[index], A[cur]); // 10 -> 40, 10 -> 20

        // Continue heapifying down from the child position
        cur = index; // 2, 4
    }

    // After extraction, the heap is now: [x, 40, 20, 30, 10, 10]
    /*
