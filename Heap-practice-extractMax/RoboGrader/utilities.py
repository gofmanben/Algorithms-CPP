###############################
#Updated utility.py: fixed misleading line number messages
###############################
...
def check_equality(output,ideal):
   ...
   for i in range(0,ideal_len): # looping over smaller file
       if output_len <= i: # output file is too small
           ...
           log = log + "The following line should be present at line "+str(output_len+1)+" of your output. But it is not.(Line count starts from one)\n"
       ...
       if output_lines[i].strip().split() != ideal_lines[i].strip().split(): # take the lines, tokenize by whitespace, and check that the resulting lists are equal
           log = log+"Error in output generation. Your program is wrong on a test case. Specifically, mismatch at line "+str(i+1)+'. (Line numbers start from 1.) \n'
