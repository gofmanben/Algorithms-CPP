...
Node* LinkedList :: deleteLast(int val)
{
    ...
    Node* last = NULL;       // last node with data == val
    Node* lastPrev = NULL;   // node before last

    // Traverse list to find last occurrence
    while (curr != NULL)
    {
        if (curr->data == val)
        {
    ...
    return last; // caller is responsible for deleting if needed
}