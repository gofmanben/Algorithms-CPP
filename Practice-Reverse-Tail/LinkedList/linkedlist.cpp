...
void LinkedList :: reverseTail(int val)
{
    ...
    // If list has fewer than 2 nodes, or val < 2 → nothing to reverse
    if (n < 2 || val < 2) return;

    // If val >= length of the list → reversing the whole list
    if (val >= n)
    {
        Node* curr = head;
        Node* prev = nullptr;
    ...
    // Walk to the node RIGHT BEFORE the last 'val' nodes.
    int steps = n - val - 1;
    Node* curr = head;
    for (int i = 0; i < steps; i++)
        curr = curr->next;

    // Reverse the tail section starting from curr->next (first node of tail part)
    Node* prev = nullptr;
    Node* tail = curr->next;

    // Reverse sublist tail -> end
    while (tail)
    {
        Node* next = tail->next;
        tail->next = prev;
    ...
    // prev is now the head of the reversed tail
    curr->next = prev;
}