...
{
    // if list is empty, simply add a new node after head
    if (head == NULL)
    {
        head = new Node;
        head->data = val;
    ...
    // otherwise, find the last node and add it after that:
    Node *curr = head; // pointer to traverse the list; staring from the first node (the node that "head" points to)
    while (curr->next != NULL)
    {
        curr = curr->next; // move to next node
    }

    // now curr points to the last Node > add a new node(with val as data) after it
