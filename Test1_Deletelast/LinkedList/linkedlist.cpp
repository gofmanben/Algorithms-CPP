...
Node* LinkedList :: deleteLast(int val)
{
    ...
    // These will store the LAST occurrence of val
    Node* lastMatch = NULL;
    Node* lastMatchPrev = NULL;

    // Walk through list and remember the last time we see val
    while (curr != NULL)
    {
        if (curr->data == val)
        {
    ...
    // If we never found val
    if (lastMatch == NULL)
        return NULL;

    // Remove lastMatch from the list
    if (lastMatchPrev == NULL)
    {
        // last occurrence is at the head
        head = head->next;
    }
    else
    {
