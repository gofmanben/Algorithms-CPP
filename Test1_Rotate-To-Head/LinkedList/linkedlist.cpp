...
{
    // Case 1: empty list or only 1 node -> no rotation possible
    if (head == NULL || head->next == NULL)
        return;

    // Case 2: val is already at head
    if (head->data == val)
        return;
    ...
    // If val not found, do nothing
    if (curr == NULL)
        return;

    // Now:
    // curr points to first node with val
    // prev points to node right before curr
    ...
    // Rotate:
    // tail should point to old head
    tail->next = head;

    // new head becomes curr
    head = curr;

    // prev becomes new tail, so break the list there
    prev->next = NULL;
}
